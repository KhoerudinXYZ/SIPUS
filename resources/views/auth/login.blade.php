...
@section('content')
<div x-data="{ loaded: false, showPassword: false }"
     x-init="setTimeout(() => loaded = true, 100)"
     :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'"
     class="transition-all duration-700 ease-out flex min-h-[78vh] flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">

    {{-- Ambient glows --}}
    <div class="pointer-events-none absolute inset-0 overflow-hidden">
        <div class="absolute top-1/4 left-1/4 w-96 h-96 bg-indigo-200/30 rounded-full blur-3xl"></div>
        <div class="absolute bottom-1/4 right-1/4 w-72 h-72 bg-violet-200/30 rounded-full blur-3xl"></div>
    </div>
        ...
        <div class="bg-white/80 backdrop-blur-xl py-8 px-6 shadow-2xl shadow-indigo-100/40 border border-white/70 rounded-3xl">
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="email" name="email" type="email" autocomplete="email" required value="{{ old('email') }}"
                               ...
                               class="block w-full rounded-xl border pl-10 pr-4 py-3 text-slate-900 text-sm placeholder:text-slate-400 shadow-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/15 transition-all bg-slate-50/80 focus:bg-white hover:bg-white {{ $errors->has('email') ? 'border-red-300 focus:border-red-400 focus:ring-red-400/15' : 'border-slate-200 focus:border-indigo-400 hover:border-slate-300' }}">
                    </div>
                    @error('email')
                        <p class="mt-1.5 text-xs text-red-600 flex items-center gap-1">
                            <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5zm0 10a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>
                            {{ $message }}
                        </p>
                    @enderror
                </div>
                ...
                <button type="submit"
                    ...
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9" /></svg>
                    Masuk ke Aplikasi
                </button>
            </form>
            ...
            <div class="mt-6 border-t border-slate-100 pt-5">
                <p class="text-xs font-semibold text-slate-400 text-center uppercase tracking-wider mb-3">Demo Login Cepat</p>
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" onclick="fillCredentials('admin@perpustakaan.com')"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-indigo-100 bg-indigo-50/50 px-3 py-2.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 transition-all">
                    ...
                    <button type="button" onclick="fillCredentials('user@perpustakaan.com')"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-violet-100 bg-violet-50/50 px-3 py-2.5 text-xs font-semibold text-violet-700 hover:bg-violet-100 transition-all">
