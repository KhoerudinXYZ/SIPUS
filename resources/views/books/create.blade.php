...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Isi informasi di bawah untuk menambahkan buku baru ke perpustakaan</p>
    </div>
        ...
        <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="author" name="author" type="text" required value="{{ old('author') }}"
                ...
                <div>
                    ...
                    <input id="publisher" name="publisher" type="text" required value="{{ old('publisher') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input id="year" name="year" type="number" required value="{{ old('year', date('Y')) }}" min="1000" max="{{ date('Y') + 1 }}"
                ...
                <div>
                    ...
                    <input id="stock" name="stock" type="number" required value="{{ old('stock', 1) }}" min="0"
            ...
            <div>
                ...
                <div id="preview-container" class="hidden mt-2 mb-3 items-center gap-4 p-2 border border-dashed border-indigo-100 bg-indigo-50/30 rounded-lg animate-fade-in">
                    <img id="image-preview" src="#" alt="Pratinjau Sampul" class="h-20 w-16 rounded-lg border border-indigo-200 object-cover shadow-sm">
                    <div class="flex flex-col">
                        <span class="text-xs font-medium text-indigo-600">Sampul terpilih:</span>
                        <span id="file-name" class="text-xs text-slate-600 font-mono truncate max-w-xs">nama_file.jpg</span>
                    </div>
                </div>

                <div class="mt-1 flex items-center justify-center w-full">
                    <label for="cover_image" class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer bg-slate-50/50 hover:bg-indigo-50/50 hover:border-indigo-200 transition-all">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="text-xs text-slate-500"><span id="upload-text" class="font-semibold text-indigo-600">Klik untuk upload</span> atau drag & drop</p>
...
function previewImage(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        if (uploadText) uploadText.textContent = 'Klik untuk upload';
    }
}
</script>
@endsection
